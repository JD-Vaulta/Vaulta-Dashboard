...
import { initDynamoDB, getLatestReading, getLatestPackControllerReading, detectBatteryType } from '../queries.js';
...
const discoverAvailableBatteries = async () => {
  try {
    ...
    try {
      const packControllerData = await getLatestPackControllerReading(docClient, "PACK-CONTROLLER");
      if (packControllerData) {
        availableBatteries.push({
          ...
          lastSeen: new Date(parseInt(packControllerData.Timestamp?.N || packControllerData.Timestamp) * 1000),
          isActive: true
        });
    ...
    for (const batteryId of commonBatteryIds) {
      try {
        ...
        if (latestReading) {
          const lastSeenTimestamp = parseInt(latestReading.Timestamp?.N || latestReading.Timestamp) * 1000;
          const lastSeen = new Date(lastSeenTimestamp);
          const hoursSinceLastSeen = (Date.now() - lastSeenTimestamp) / (1000 * 60 * 60);
    ...
    console.log(`[BatteryContext] Discovery complete: found ${availableBatteries.length} available batteries`);
...
export const BatteryProvider = ({ children, user }) => {
  ...
  const loadUserBatteries = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('[BatteryContext] Loaded registered batteries:', activeBatteries.length, 'active batteries');
        ...
        if (activeBatteries.length > 0 && !selectedBattery) {
          ...
          console.log('[BatteryContext] Auto-selected first registered battery:', activeBatteries[0].batteryId);
        }
  ...
  const loadAvailableSystemBatteries = async () => {
    ...
    try {
      ...
      console.log('[BatteryContext] Loaded system batteries:', discoveredBatteries.length, 'available');
  ...
  useEffect(() => {
    if ((userBatteries.length > 0 || availableSystemBatteries.length > 0) && !selectedBattery) {
      const savedBatteryId = localStorage.getItem('selectedBatteryId');
      if (savedBatteryId) {
        // Check registered batteries first
        let savedBattery = userBatteries.find(b => b.batteryId === savedBatteryId);
        
        // If not found, check system batteries
        if (!savedBattery) {
          savedBattery = availableSystemBatteries.find(b => b.id === savedBatteryId);
        }
  ...
  const getBatteryOptions = () => {
    ...
    const systemOptions = availableSystemBatteries
      .filter(sysBattery => !userBatteries.find(userBattery => userBattery.batteryId === sysBattery.id))
      .map(sysBattery => ({
        value: sysBattery.id,
  ...
  const getAllAvailableBatteries = () => {
    ...
    userBatteries.forEach(battery => {
      allBatteries.push({
        ...
        name: battery.nickname || battery.serialNumber || `Battery ${battery.batteryId}`,
        type: detectBatteryType(battery.batteryId),
        source: 'registered',
        displayName: `${battery.nickname || battery.serialNumber} (${battery.batteryId})`,
    ...
    availableSystemBatteries.forEach(battery => {
      const isAlreadyRegistered = userBatteries.find(userBat => userBat.batteryId === battery.id);
  ...
  const contextValue = {
    ...
    // Computed values
    selectedBatteryId: selectedBattery?.batteryId || selectedBattery?.id || null,
    selectedBatteryNickname: selectedBattery?.nickname || selectedBattery?.name || null,
    batteryCount: userBatteries.length,
    systemBatteryCount: availableSystemBatteries.length,
    totalAvailableBatteries: userBatteries.length + availableSystemBatteries.filter(
      sysBattery => !userBatteries.find(userBattery => userBattery.batteryId === sysBattery.id)
    ).length
  };
